...
var bcryptPassword = require('bcrypt');
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            ...
                            return [3 /*break*/, 3];
                        case 1: return [4 /*yield*/, bcryptPassword.compare(req.body.password, user.password)];
                        case 2:
                            ...
                            if (!passwordMatch) {
                                ...
                                token_1 = jwt.sign({ user: user.email }, 'secret_key', {
                                    expiresIn: '24hr'
                                });
                                user.loginToken = token_1;
                                user.save(function (err, user) {
                                    if (err) {
                                        res.status(500).send();
                                    }
                                    else {
                                        res.status(200).send({ loginToken: token_1, message: 'Successful login', user: user });
                                    }
                                });
                            }
                            _a.label = 3;
